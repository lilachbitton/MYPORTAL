...
import { collection, getDocs, updateDoc, doc, getDoc, setDoc, query, orderBy } from "firebase/firestore";
...
כניסה למערכת: https://login-portal-peach.vercel.app/`,
 ...
 const getCurrentUserCycle = async (userId) => {
   const docSnap = await getDoc(doc(db, "users", userId));
   return docSnap.data()?.cycle || null;
 };
 ...
 const toggleUserStatus = async (userId, currentStatus) => {
   try {
     await updateDoc(doc(db, "users", userId), {
       ...
       cycle: !currentStatus ? null : await getCurrentUserCycle(userId)
     });
     await fetchUsers();
   } catch (error) {
           ...
           {getSortedUsers().map(user => (
             <tr key={user.id} className="hover:bg-gray-50">
               ...
               <td className="border p-2">
                 <select
                   value={user.cycle || ''}
                   onChange={(e) => updateUserCycle(user.id, e.target.value)}
